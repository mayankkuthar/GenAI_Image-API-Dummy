...
async def process_image(file: UploadFile = File(...), document_type: Optional[str] = Form(None)):
    ...
    if not any(filename.endswith(ext) for ext in ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.tiff', '.pdf']):
        raise HTTPException(status_code=400, detail="Invalid file extension. Please upload an image or PDF file.")
